    ...
    <div class="flex h-full min-h-0 flex-col">
        <div class="flex shrink-0 items-start justify-between border-b border-slate-100 px-5 pb-4 pt-5">
            <div class="flex items-center gap-3">
                <div class="flex h-11 w-11 items-center justify-center rounded-lg bg-blue-50 text-blue-600" aria-hidden="true">
                ...
                <div>
                    <h2 id="cartDrawerTitle" class="m-0 text-xl font-extrabold text-blue-600">Cart</h2>
                    <p class="cart-drawer-sub mt-0.5 text-sm text-slate-500">{{ $cartCount }} {{ $cartCount === 1 ? 'item' : 'items' }}</p>
                </div>
            </div>
            <button type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-white text-2xl text-slate-700 hover:bg-slate-50" id="cartDrawerClose" aria-label="Close cart">&times;</button>
        </div>
