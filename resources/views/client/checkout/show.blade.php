...
@section('content')
    ...
    <div class="grid grid-cols-1 items-start gap-6 lg:grid-cols-[1fr_340px]">
        <div class="rounded-xl border border-slate-200 bg-white p-4.5">
            <h2 class="mb-3.5 text-base font-extrabold">Shipping addresses</h2>
            ...
            @else
                <div class="mb-4.5 grid gap-3">
                    @foreach ($addresses as $address)
                        <div class="rounded-xl border bg-slate-50/70 p-3 {{ $address->is_default ? 'border-[#f16743] ring-1 ring-[#f16743]/20' : 'border-slate-200' }}">
                            ...
                            <form method="POST" action="{{ route('client.addresses.update', $address) }}">
                                ...
                                <div class="mb-3 mt-2 flex items-center gap-2">
                                    <input id="checkout-default-{{ $address->id }}" type="checkbox" name="is_default" value="1" class="h-4 w-4 rounded border-slate-300 text-[#f16743] focus:ring-[#f16743]/30" {{ $address->is_default ? 'checked' : '' }}>
                                    <label class="text-sm text-slate-700" for="checkout-default-{{ $address->id }}">Set as default address</label>
                                </div>

                                <button type="submit" class="rounded-lg bg-slate-900 px-3 py-2.5 text-sm font-bold text-white">Update address</button>
                            </form>
                        </div>
                    @endforeach
                </div>
            ...
            <h2 class="mb-3.5 mt-4.5 text-base font-extrabold">Add new address</h2>
                ...
                <form method="POST" action="{{ route('client.addresses.store') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                <span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400/70"><svg class="h-[18px] w-[18px]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.1" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4 7h16M4 12h10m-10 5h16" /></svg></span>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                <span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400/70"><svg class="h-[18px] w-[18px]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.1" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M5 4h4l2 5-2 2a16 16 0 0 0 4 4l2-2 5 2v4a2 2 0 0 1-2 2h-1C10.8 21 3 13.2 3 6V5a2 2 0 0 1 2-1z" /></svg></span>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                <span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400/70"><svg class="h-[18px] w-[18px]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.1" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21s7-5.6 7-11a7 7 0 1 0-14 0c0 5.4 7 11 7 11z" /><circle cx="12" cy="10" r="2.6" /></svg></span>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                <span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400/70"><svg class="h-[18px] w-[18px]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.1" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4 20h16M6 20V7l6-3 6 3v13M9 10h.01M9 13h.01M9 16h.01M15 10h.01M15 13h.01M15 16h.01" /></svg></span>
                    ...
                    <label class="mt-3 inline-flex cursor-pointer items-center gap-2 text-sm text-slate-700" for="checkout-new-default">
                        <input id="checkout-new-default" type="checkbox" name="is_default" value="1" class="h-4 w-4 rounded border-slate-300 text-[#f16743] focus:ring-[#f16743]/30">
            ...
            <h2 class="mb-3 mt-5.5 text-base font-extrabold">Place order</h2>
            <form method="POST" action="{{ route('client.checkout.place-order') }}" class="mt-2">
                @csrf
                <label class="mb-1.5 block text-sm font-semibold text-slate-700" for="address_id">Delivery address</label>
                <select id="address_id" name="address_id" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" required>
                    <option value="">Select address</option>
                    @foreach ($addresses as $address)
                        <option value="{{ $address->id }}" {{ old('address_id', $defaultAddressId) === $address->id ? 'selected' : '' }}>
                            {{ $address->label ?: 'Address' }} - {{ $address->full_name }} ({{ $address->city }})
                        </option>
                    @endforeach
                </select>
                ...
                <button
                    type="submit"
                    class="mt-4 w-full rounded-lg bg-[#f16743] px-3 py-3 text-[15px] font-extrabold text-white disabled:cursor-not-allowed disabled:opacity-60"
                    {{ $addresses->isEmpty() ? 'disabled' : '' }}
                >
                    Place order
                </button>
            </form>
            <a class="mt-3.5 inline-block text-sm font-bold text-[#f16743]" href="{{ route('client.categories.index') }}">&larr; Continue shopping</a>
        </div>
