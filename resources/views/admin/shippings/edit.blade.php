        ...
        <form method="POST" action="{{ route('admin.shippings.update', $shipping) }}">
            ...
            <div class="mb-3">
                ...
                <select class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="order_id" name="order_id" required>
                    ...
                    @foreach ($orders as $order)
                        <option value="{{ $order['id'] }}" {{ old('order_id', $shipping->order_id) === $order['id'] ? 'selected' : '' }}>{{ $order['order_number'] }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div class="mb-3">
                    ...
                    <input class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="carrier" type="text" name="carrier" value="{{ old('carrier', $shipping->carrier) }}">
                </div>
                <div class="mb-3">
                    <label class="mb-1.5 block text-sm font-bold" for="tracking_number">Tracking Number</label>
                    <input class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="tracking_number" type="text" name="tracking_number" value="{{ old('tracking_number', $shipping->tracking_number) }}">
                </div>
            </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="status" name="status" required>
                        @foreach ($statuses as $status)
                            <option value="{{ $status }}" {{ old('status', $shipping->status) === $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    <label class="mb-1.5 block text-sm font-bold" for="shipped_at">Shipped At</label>
                    <input class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="shipped_at" type="datetime-local" name="shipped_at" value="{{ old('shipped_at', optional($shipping->shipped_at)->format('Y-m-d\TH:i')) }}">
                </div>
            </div>

            <div class="mb-4">
                <label class="mb-1.5 block text-sm font-bold" for="delivered_at">Delivered At</label>
                <input class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="delivered_at" type="datetime-local" name="delivered_at" value="{{ old('delivered_at', optional($shipping->delivered_at)->format('Y-m-d\TH:i')) }}">
            </div>
