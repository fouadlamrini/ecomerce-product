    ...
    <div class="max-w-[760px]">
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3.5">
                ...
                <input class="w-full rounded-lg border border-slate-300 px-3 py-2.5" id="name" type="text" name="name" value="{{ old('name', $category->name) }}" required>
            </div>
            ...
            <div class="mb-3.5">
                ...
                @if ($category->bg_image)
                    <div class="mt-2">
                        <img class="h-[100px] w-[180px] rounded-lg border border-slate-200 object-cover" src="{{ asset('storage/'.$category->bg_image) }}" alt="{{ $category->name }}">
                    </div>
                @endif
            </div>
            ...
            <div class="mb-4">
                <label class="inline-flex items-center gap-2 text-sm">
                    <input class="h-4 w-4 rounded border-slate-300 text-[#f16743] focus:ring-[#f16743]/30" type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }}>
                    Active
                </label>
            </div>
