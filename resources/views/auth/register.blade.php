...
<body class="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_12%_18%,rgba(255,127,80,0.42),transparent_44%),radial-gradient(circle_at_84%_16%,rgba(214,223,255,0.92),transparent_42%),radial-gradient(circle_at_70%_82%,rgba(255,191,170,0.55),transparent_38%),linear-gradient(155deg,#f6f7ff_0%,#f9f2ff_55%,#eef3ff_100%)] font-['Geist','Inter',Arial,sans-serif] text-slate-800">
    ...
    @if (session('success'))
        <div id="registerToast" class="fixed right-5 top-5 z-20 rounded-xl border border-emerald-200 bg-emerald-50 px-3.5 py-3 text-[13px] text-emerald-800 shadow-lg transition">{{ session('success') }}</div>
    @endif
                ...
                <div class="w-full max-w-[410px]">
                    ...
                    <form method="POST" action="{{ route('register.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="name" class="w-full rounded-xl border border-slate-200 bg-white/90 px-3.5 py-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-[#ff7f50] focus:ring-4 focus:ring-[#ff7f50]/20" type="text" name="name" value="{{ old('name') }}" placeholder="Your full name" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-4">
                            ...
                            <input id="password_confirmation" class="w-full rounded-xl border border-slate-200 bg-white/90 px-3.5 py-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-[#ff7f50] focus:ring-4 focus:ring-[#ff7f50]/20" type="password" name="password_confirmation" placeholder="••••••••" required>
                        </div>
                        <button class="w-full rounded-2xl bg-linear-to-b from-[#ff996f] via-[#ff7f50] to-[#f16743] px-6 py-3 text-sm font-extrabold text-white shadow-lg shadow-[#f16743]/30 transition hover:scale-[1.01]" type="submit">Sign Up</button>
                    </form>

                    <p class="mt-3.5 text-[13px] text-slate-600">
                        I already have an account?
                        <a class="font-bold text-[#f16743]" href="{{ route('login') }}">Sign in</a>
                    </p>
                </div>
            </div>
